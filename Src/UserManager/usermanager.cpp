...
{
    QString query("select * from PatientMsg");
...
void UserManager::slotSelectItemClicked(QListWidgetItem *item)
{
...
void UserManager::on_search_Btn_clicked()
{
    ...
    QString query(QString("select * from PatientMsg where name = '%1' or id = '%2'").arg(msg).arg(msg));
    ...
    connect(listWidget,&QListWidget::itemClicked,this,&UserManager::slotSelectItemClicked);
...
void UserManager::showUserData(uint16_t pageIndex)
{
    ...
    {
        //添加 “添加”Item
        QListWidgetItem *addItem = new QListWidgetItem(QIcon(":/images/addUser.png"),tr("添加用户"));
